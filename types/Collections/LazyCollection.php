...
assertType('Hypervel\Support\LazyCollection<string, Hypervel\Support\Collection<int, User>>', $collection->groupBy(function ($user, $int) {
    assertType('User', $user);
    assertType('int', $int);

    return 'foo';
}));
assertType('Hypervel\Support\LazyCollection<int, Hypervel\Support\Collection<int, User>>', $collection->groupBy(static fn ($user) => 0));
...
assertType('Hypervel\Support\LazyCollection<string, Hypervel\Support\Collection<string, User>>', $collection->keyBy(fn ($user) => 'bar')->groupBy(function ($user) {
    return 'foo';
}, preserveKeys: true));

assertType('Hypervel\Support\LazyCollection<(int|string), User>', $collection->keyBy('name'));
assertType('Hypervel\Support\LazyCollection<string, User>', $collection->keyBy(function ($user, $int) {
    assertType('User', $user);
    assertType('int', $int);

    return 'foo';
}));
assertType('Hypervel\Support\LazyCollection<int, User>', $collection->keyBy(static fn ($user): int => 0));
