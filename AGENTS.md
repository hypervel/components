...
- **Prefer `timestamp` over `timestampTz` in migrations** — store times in UTC with plain `timestamp` columns, matching the normal convention in Laravel and Hypervel first-party migrations. Use `dateTime` instead of `timestamp` for date-time values supplied by users or external systems. On MySQL and MariaDB, `timestamp` only safely covers 1970–2038; `dateTime` avoids that limit and works consistently across all supported databases. Reserve `timestampTz` for columns that genuinely need database-level timezone semantics, such as integrating with an existing timezone-aware schema or columns written by clients in different session timezones. The schema API supports all three — this is a column-choice convention, not an API restriction.
