...
- Any cache retained across requests in a worker—for example, in static properties or properties on singleton instances—must either have a naturally limited set of keys or deliberately discard entries that can safely be recomputed. Do not add a size limit merely to hide accidental growth from request- or user-derived keys.
