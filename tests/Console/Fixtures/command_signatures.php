<?php

declare(strict_types=1);

/*
 * This file records command names, aliases, arguments, and options. It exists
 * to catch regressions if a command's public signature (name, argument, or option
 * definitions) accidentally changes - most notably during a refactor that
 * consolidates a command's $name/getArguments()/getOptions() into a single
 * $signature string.
 *
 * dispatcherOptionOffset records the shared option's position: array-based
 * commands put it first, and isolation adds --isolated after it.
 *
 * Update this file whenever a command's signature intentionally changes.
 */
return [
    \Hypervel\Auth\Console\ClearResetsCommand::class => [
        'name' => 'auth:clear-resets',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the password broker (defaults to the current guard\'s broker)',
            ],
        ],
    ],
    \Hypervel\Cache\Console\CacheTableCommand::class => [
        'name' => 'make:cache-table',
        'aliases' => [
            'cache:table',
        ],
    ],
    \Hypervel\Cache\Console\ClearCommand::class => [
        'name' => 'cache:clear',
        'arguments' => [
            [
                'name' => 'store',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the store you would like to clear',
            ],
        ],
        'options' => [
            [
                'name' => 'tags',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The cache tags you would like to clear',
            ],
            [
                'name' => 'locks',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Only clear cache locks',
            ],
        ],
    ],
    \Hypervel\Cache\Console\ForgetCommand::class => [
        'name' => 'cache:forget',
        'arguments' => [
            [
                'name' => 'key',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The key to remove',
            ],
            [
                'name' => 'store',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The store to remove the key from',
            ],
        ],
    ],
    \Hypervel\Cache\Console\PruneStaleTagsCommand::class => [
        'name' => 'cache:prune-stale-tags',
        'arguments' => [
            [
                'name' => 'store',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the store you would like to prune tags from',
            ],
        ],
    ],
    \Hypervel\Concurrency\Console\InvokeSerializedClosureCommand::class => [
        'name' => 'invoke-serialized-closure',
        'hidden' => true,
        'arguments' => [
            [
                'name' => 'code',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The serialized closure',
            ],
        ],
    ],
    \Hypervel\Console\Commands\ScheduleClearCacheCommand::class => [
        'name' => 'schedule:clear-cache',
    ],
    // REMOVED: schedule:finish; coroutine tasks finish directly through Event::finish().
    \Hypervel\Console\Commands\ScheduleInterruptCommand::class => [
        'name' => 'schedule:interrupt',
        'options' => [
            [
                'name' => 'minutes',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '1',
                'description' => 'TTL in minutes for the interrupt signal (minimum 1)',
            ],
        ],
    ],
    \Hypervel\Console\Commands\ScheduleListCommand::class => [
        'name' => 'schedule:list',
        'options' => [
            [
                'name' => 'timezone',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The timezone that times should be displayed in',
            ],
            [
                'name' => 'environment',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'Display the tasks scheduled to run on this environment',
            ],
            [
                'name' => 'next',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Sort the listed tasks by their next due date',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the scheduled tasks as JSON',
            ],
        ],
    ],
    \Hypervel\Console\Commands\SchedulePauseCommand::class => [
        'name' => 'schedule:pause',
    ],
    \Hypervel\Console\Commands\ScheduleResumeCommand::class => [
        'name' => 'schedule:resume',
        'aliases' => [
            'schedule:continue',
        ],
    ],
    \Hypervel\Console\Commands\ScheduleRunCommand::class => [
        'name' => 'schedule:run',
        'options' => [
            [
                'name' => 'once',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Run only once without looping',
            ],
            [
                'name' => 'concurrency',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '60',
                'description' => 'The number of background tasks to process at once',
            ],
            [
                'name' => 'whisper',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Do not output message indicating that no commands were ready to run',
            ],
        ],
    ],
    \Hypervel\Console\Commands\ScheduleTestCommand::class => [
        'name' => 'schedule:test',
        'options' => [
            [
                'name' => 'name',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The name of the scheduled command to run',
            ],
        ],
    ],
    // REMOVED: schedule:work; schedule:run already runs continuously in Hypervel.
    \Hypervel\Database\Console\DbCommand::class => [
        'name' => 'db',
        'arguments' => [
            [
                'name' => 'connection',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The database connection that should be used',
            ],
        ],
        'options' => [
            [
                'name' => 'read',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Connect to the read connection',
            ],
            [
                'name' => 'write',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Connect to the write connection',
            ],
            // REMOVED: --pooled; external pooler endpoints are selected by connection name.
        ],
    ],
    \Hypervel\Database\Console\DumpCommand::class => [
        'name' => 'schema:dump',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The path where the schema dump file should be stored',
            ],
            [
                'name' => 'prune',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Delete all existing migration files',
            ],
            [
                'name' => 'without-migration-data',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Dump the schema without the migration data',
            ],
        ],
    ],
    \Hypervel\Database\Console\Factories\FactoryMakeCommand::class => [
        'name' => 'make:factory',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the factory',
            ],
        ],
        'options' => [
            [
                'name' => 'model',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The name of the model',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\FreshCommand::class => [
        'name' => 'migrate:fresh',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The default database connection to use',
            ],
            [
                'name' => 'drop-views',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Drop all tables and views',
            ],
            [
                'name' => 'drop-types',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Drop all tables and types (Postgres only)',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'The path(s) to the migrations files to be executed',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
            [
                'name' => 'schema-path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The path to a schema dump file',
            ],
            [
                'name' => 'seed',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the seed task should be re-run',
            ],
            [
                'name' => 'seeder',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The class name of the root seeder',
            ],
            [
                'name' => 'step',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the migrations to be run so they can be rolled back individually',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\InstallCommand::class => [
        'name' => 'migrate:install',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\MigrateCommand::class => [
        'name' => 'migrate',
        'dispatcherOptionOffset' => 10,
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The default database connection to use',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'The path(s) to the migrations files to be executed',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
            [
                'name' => 'schema-path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The path to a schema dump file',
            ],
            [
                'name' => 'pretend',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Dump the SQL queries that would be run',
            ],
            [
                'name' => 'seed',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the seed task should be re-run',
            ],
            [
                'name' => 'seeder',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The class name of the root seeder',
            ],
            [
                'name' => 'step',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the migrations to be run so they can be rolled back individually',
            ],
            [
                'name' => 'graceful',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Return a successful exit code even if an error occurs',
            ],
            [
                'name' => 'isolated',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => false,
                'description' => 'Do not run the command if another instance of the command is already running',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\MigrateMakeCommand::class => [
        'name' => 'make:migration',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the migration',
            ],
        ],
        'options' => [
            [
                'name' => 'create',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The table to be created',
            ],
            [
                'name' => 'table',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The table to migrate',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The location where the migration file should be created',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
            // REMOVED: deprecated --fullpath option; migration output uses the current command format.
        ],
    ],
    \Hypervel\Database\Console\Migrations\RefreshCommand::class => [
        'name' => 'migrate:refresh',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'The path(s) to the migrations files to be executed',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
            [
                'name' => 'seed',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the seed task should be re-run',
            ],
            [
                'name' => 'seeder',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The class name of the root seeder',
            ],
            [
                'name' => 'step',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of migrations to be reverted & re-run',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\ResetCommand::class => [
        'name' => 'migrate:reset',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'The path(s) to the migrations files to be executed',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
            [
                'name' => 'pretend',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Dump the SQL queries that would be run',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\RollbackCommand::class => [
        'name' => 'migrate:rollback',
        'dispatcherOptionOffset' => 0,
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'The path(s) to the migrations files to be executed',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
            [
                'name' => 'pretend',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Dump the SQL queries that would be run',
            ],
            [
                'name' => 'step',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of migrations to be reverted',
            ],
            [
                'name' => 'batch',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => true,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The batch of migrations (identified by their batch number) to be reverted',
            ],
        ],
    ],
    \Hypervel\Database\Console\Migrations\StatusCommand::class => [
        'name' => 'migrate:status',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'pending',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => false,
                'description' => 'Only list pending migrations',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'The path(s) to the migrations files to use',
            ],
            [
                'name' => 'realpath',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate any provided migration file paths are pre-resolved absolute paths',
            ],
        ],
    ],
    \Hypervel\Database\Console\MonitorCommand::class => [
        'name' => 'db:monitor',
        'options' => [
            [
                'name' => 'databases',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connections to monitor',
            ],
            [
                'name' => 'max',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The maximum number of connections that can be open before an event is dispatched',
            ],
        ],
    ],
    \Hypervel\Database\Console\PruneCommand::class => [
        'name' => 'model:prune',
        'options' => [
            [
                'name' => 'model',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'Class names of the models to be pruned',
            ],
            [
                'name' => 'except',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'Class names of the models to be excluded from pruning',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'Absolute path(s) to directories where models are located',
            ],
            [
                'name' => 'chunk',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '1000',
                'description' => 'The number of models to retrieve per chunk of models to be deleted',
            ],
            [
                'name' => 'pretend',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Display the number of prunable records found instead of deleting them',
            ],
        ],
    ],
    \Hypervel\Database\Console\Seeds\SeedCommand::class => [
        'name' => 'db:seed',
        'arguments' => [
            [
                'name' => 'class',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The class name of the root seeder',
            ],
        ],
        'options' => [
            [
                'name' => 'class',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'Database\Seeders\DatabaseSeeder',
                'description' => 'The class name of the root seeder',
            ],
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to seed',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
        ],
    ],
    \Hypervel\Database\Console\Seeds\SeederMakeCommand::class => [
        'name' => 'make:seeder',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the seeder',
            ],
        ],
    ],
    \Hypervel\Database\Console\ShowCommand::class => [
        'name' => 'db:show',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the database information as JSON',
            ],
            [
                'name' => 'counts',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Show the table row count <bg=red;options=bold> Note: This can be slow on large databases </>',
            ],
            [
                'name' => 'views',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Show the database views <bg=red;options=bold> Note: This can be slow on large databases </>',
            ],
            [
                'name' => 'types',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Show the user defined types',
            ],
        ],
    ],
    \Hypervel\Database\Console\ShowModelCommand::class => [
        'name' => 'model:show',
        'arguments' => [
            [
                'name' => 'model',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The model to show',
            ],
        ],
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the model as JSON',
            ],
        ],
    ],
    \Hypervel\Database\Console\TableCommand::class => [
        'name' => 'db:table',
        'arguments' => [
            [
                'name' => 'table',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the table',
            ],
        ],
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the table information as JSON',
            ],
        ],
    ],
    \Hypervel\Database\Console\WipeCommand::class => [
        'name' => 'db:wipe',
        'options' => [
            [
                'name' => 'database',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The database connection to use',
            ],
            [
                'name' => 'drop-views',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Drop all tables and views',
            ],
            [
                'name' => 'drop-types',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Drop all tables and types (Postgres only)',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\AboutCommand::class => [
        'name' => 'about',
        'options' => [
            [
                'name' => 'only',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The section to display',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the information as JSON',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ApiInstallCommand::class => [
        'name' => 'install:api',
        'options' => [
            [
                'name' => 'composer',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'global',
                'description' => 'Absolute path to the Composer binary which should be used to install packages',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite any existing API routes file',
            ],
            // REMOVED: Passport installation coverage belongs to the unported package tracked in docs/todo.md.
            [
                'name' => 'without-migration-prompt',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Do not prompt to run pending migrations',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\BroadcastingInstallCommand::class => [
        'name' => 'install:broadcasting',
        'options' => [
            [
                'name' => 'composer',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'global',
                'description' => 'Absolute path to the Composer binary which should be used to install packages',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite any existing broadcasting routes file',
            ],
            [
                'name' => 'without-reverb',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Do not prompt to install Hypervel Reverb',
            ],
            [
                'name' => 'reverb',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Install Hypervel Reverb as the default broadcaster',
            ],
            [
                'name' => 'pusher',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Install Pusher as the default broadcaster',
            ],
            [
                'name' => 'ably',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Install Ably as the default broadcaster',
            ],
            [
                'name' => 'without-node',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Do not prompt to install Node dependencies',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\CastMakeCommand::class => [
        'name' => 'make:cast',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the cast',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the cast already exists',
            ],
            [
                'name' => 'inbound',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate an inbound cast class',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ChannelListCommand::class => [
        'name' => 'channel:list',
    ],
    \Hypervel\Foundation\Console\ChannelMakeCommand::class => [
        'name' => 'make:channel',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the channel',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the channel already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ClassMakeCommand::class => [
        'name' => 'make:class',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the class',
            ],
        ],
        'options' => [
            [
                'name' => 'invokable',
                'shortcut' => 'i',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a single method, invokable class',
            ],
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the class already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ClearCompiledCommand::class => [
        'name' => 'clear-compiled',
    ],
    \Hypervel\Foundation\Console\ComponentMakeCommand::class => [
        'name' => 'make:component',
        'dispatcherOptionOffset' => 0,
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the component',
            ],
        ],
        'options' => [
            [
                'name' => 'inline',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a component that renders an inline view',
            ],
            [
                'name' => 'view',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create an anonymous component with only a view',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => true,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The location where the component view should be created',
            ],
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the component already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ConfigCacheCommand::class => [
        'name' => 'config:cache',
        'options' => [
            [
                'name' => 'dump-to',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Internal option used to dump fresh configuration to a temporary file',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ConfigClearCommand::class => [
        'name' => 'config:clear',
    ],
    \Hypervel\Foundation\Console\ConfigMakeCommand::class => [
        'name' => 'make:config',
        'aliases' => [
            'config:make',
        ],
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the config',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the configuration file even if it already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ConfigPublishCommand::class => [
        'name' => 'config:publish',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the configuration file to publish',
            ],
        ],
        'options' => [
            [
                'name' => 'all',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Publish all configuration files',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite any existing configuration files',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ConfigShowCommand::class => [
        'name' => 'config:show',
        'arguments' => [
            [
                'name' => 'config',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The configuration file or key to show',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ConsoleMakeCommand::class => [
        'name' => 'make:command',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the command',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the console command already exists',
            ],
            [
                'name' => 'command',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The terminal command that will be used to invoke the class',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\DevCommand::class => [
        'name' => 'dev',
        'options' => [
            [
                'name' => 'stream',
                'shortcut' => 's',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Start in stream mode',
            ],
            [
                'name' => 'tabs',
                'shortcut' => 't',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Start in tabs mode',
            ],
            [
                'name' => 'inline',
                'shortcut' => 'i',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Print output inline instead of rendering the TUI (the default when not a TTY)',
            ],
            [
                'name' => 'timestamps',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Display timestamps on each output line',
            ],
            [
                'name' => 'no-restart',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Disable auto-restart on crash',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Emit newline-delimited JSON events. Implies --inline',
            ],
            [
                'name' => 'buffer-size',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Set the max lines per command buffer',
            ],
            [
                'name' => 'stream-buffer-size',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Set the max lines in the stream buffer',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\DevListCommand::class => [
        'name' => 'dev:list',
        'dispatcherOptionOffset' => 0,
        'options' => [
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the dev process list as JSON',
            ],
            [
                'name' => 'filter',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => true,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the dev processes by name or command',
            ],
            [
                'name' => 'except-vendor',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Do not display dev processes registered by vendor packages',
            ],
            [
                'name' => 'only-vendor',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Only display dev processes registered by vendor packages',
            ],
        ],
    ],
    // REMOVED: docs opens Laravel's website; Hypervel documentation is available at hypervel.org.
    \Hypervel\Foundation\Console\DownCommand::class => [
        'name' => 'down',
        'options' => [
            [
                'name' => 'redirect',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The path that users should be redirected to',
            ],
            [
                'name' => 'render',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The view that should be prerendered for display during maintenance mode',
            ],
            [
                'name' => 'retry',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of seconds or the datetime after which the request may be retried',
            ],
            [
                'name' => 'refresh',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of seconds after which the browser may refresh',
            ],
            [
                'name' => 'secret',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The secret phrase that may be used to bypass maintenance mode',
            ],
            [
                'name' => 'with-secret',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a random secret phrase that may be used to bypass maintenance mode',
            ],
            [
                'name' => 'status',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '503',
                'description' => 'The status code that should be used when returning the maintenance mode response',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\EnumMakeCommand::class => [
        'name' => 'make:enum',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the enum',
            ],
        ],
        'options' => [
            [
                'name' => 'string',
                'shortcut' => 's',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a string backed enum.',
            ],
            [
                'name' => 'int',
                'shortcut' => 'i',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate an integer backed enum.',
            ],
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the enum even if the enum already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\EnvironmentCommand::class => [
        'name' => 'env',
    ],
    \Hypervel\Foundation\Console\EnvironmentDecryptCommand::class => [
        'name' => 'env:decrypt',
        'options' => [
            [
                'name' => 'key',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The encryption key',
            ],
            [
                'name' => 'cipher',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The encryption cipher',
            ],
            [
                'name' => 'env',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The environment to be decrypted',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite the existing environment file',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Path to write the decrypted file',
            ],
            [
                'name' => 'filename',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filename of the decrypted file',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\EnvironmentEncryptCommand::class => [
        'name' => 'env:encrypt',
        'options' => [
            [
                'name' => 'key',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The encryption key',
            ],
            [
                'name' => 'cipher',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The encryption cipher',
            ],
            [
                'name' => 'env',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The environment to be encrypted',
            ],
            [
                'name' => 'readable',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Encrypt each variable individually with readable names, updating existing files and preserving unchanged values',
            ],
            [
                'name' => 'prune',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Delete the original environment file',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Re-encrypt all values, overwriting the existing encrypted environment file',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\EventCacheCommand::class => [
        'name' => 'event:cache',
    ],
    \Hypervel\Foundation\Console\EventClearCommand::class => [
        'name' => 'event:clear',
    ],
    \Hypervel\Foundation\Console\EventGenerateCommand::class => [
        'name' => 'event:generate',
        'hidden' => true,
    ],
    \Hypervel\Foundation\Console\EventListCommand::class => [
        'name' => 'event:list',
        'options' => [
            [
                'name' => 'event',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the events by name',
            ],
            [
                'name' => 'listener',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the events by listener name',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the events and listeners as JSON',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\EventMakeCommand::class => [
        'name' => 'make:event',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the event',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the event already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ExceptionMakeCommand::class => [
        'name' => 'make:exception',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the exception',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the exception already exists',
            ],
            [
                'name' => 'render',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the exception with an empty render method',
            ],
            [
                'name' => 'report',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the exception with an empty report method',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\InterfaceMakeCommand::class => [
        'name' => 'make:interface',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the interface',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the interface even if the interface already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\JobMakeCommand::class => [
        'name' => 'make:job',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the job',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the job already exists',
            ],
            [
                'name' => 'sync',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates that the job should be synchronous',
            ],
            [
                'name' => 'batched',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates that the job should be batchable',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\JobMiddlewareMakeCommand::class => [
        'name' => 'make:job-middleware',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the middleware',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the job middleware already exists',
            ],
        ],
    ],
    \Hypervel\Encryption\Commands\KeyGenerateCommand::class => [
        'name' => 'key:generate',
        'options' => [
            [
                'name' => 'show',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Display the key instead of modifying files',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\LangPublishCommand::class => [
        'name' => 'lang:publish',
        'options' => [
            [
                'name' => 'existing',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Publish and overwrite only the files that have already been published',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite any existing files',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ListenerMakeCommand::class => [
        'name' => 'make:listener',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the listener',
            ],
        ],
        'options' => [
            [
                'name' => 'event',
                'shortcut' => 'e',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The event class being listened for',
            ],
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the listener already exists',
            ],
            [
                'name' => 'queued',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates the event listener should be queued',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\MailMakeCommand::class => [
        'name' => 'make:mail',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the mailable',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the mailable already exists',
            ],
            [
                'name' => 'markdown',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => false,
                'description' => 'Create a new Markdown template for the mailable',
            ],
            [
                'name' => 'view',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => false,
                'description' => 'Create a new Blade template for the mailable',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ModelMakeCommand::class => [
        'name' => 'make:model',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the model',
            ],
        ],
        'options' => [
            [
                'name' => 'all',
                'shortcut' => 'a',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a migration, seeder, factory, policy, resource controller, and form request classes for the model',
            ],
            [
                'name' => 'controller',
                'shortcut' => 'c',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a new controller for the model',
            ],
            [
                'name' => 'factory',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a new factory for the model',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the model already exists',
            ],
            [
                'name' => 'migration',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a new migration file for the model',
            ],
            [
                'name' => 'morph-pivot',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the generated model should be a custom polymorphic intermediate table model',
            ],
            [
                'name' => 'policy',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a new policy for the model',
            ],
            [
                'name' => 'seed',
                'shortcut' => 's',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a new seeder for the model',
            ],
            [
                'name' => 'pivot',
                'shortcut' => 'p',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the generated model should be a custom intermediate table model',
            ],
            [
                'name' => 'resource',
                'shortcut' => 'r',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the generated controller should be a resource controller',
            ],
            [
                'name' => 'api',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicates if the generated controller should be an API resource controller',
            ],
            [
                'name' => 'requests',
                'shortcut' => 'R',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create new form request classes and use them in the resource controller',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\NotificationMakeCommand::class => [
        'name' => 'make:notification',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the notification',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the notification already exists',
            ],
            [
                'name' => 'markdown',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Create a new Markdown template for the notification',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ObserverMakeCommand::class => [
        'name' => 'make:observer',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the observer',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the observer already exists',
            ],
            [
                'name' => 'model',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The model that the observer applies to',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\OptimizeClearCommand::class => [
        'name' => 'optimize:clear',
        'options' => [
            [
                'name' => 'except',
                'shortcut' => 'e',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The commands to skip',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\OptimizeCommand::class => [
        'name' => 'optimize',
        'options' => [
            [
                'name' => 'except',
                'shortcut' => 'e',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Do not run the commands matching the key or name',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\PackageDiscoverCommand::class => [
        'name' => 'package:discover',
    ],
    \Hypervel\Foundation\Console\PolicyMakeCommand::class => [
        'name' => 'make:policy',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the policy',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the policy already exists',
            ],
            [
                'name' => 'model',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The model that the policy applies to',
            ],
            [
                'name' => 'guard',
                'shortcut' => 'g',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The guard that the policy relies on',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ProviderMakeCommand::class => [
        'name' => 'make:provider',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the provider',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the provider already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ReloadCommand::class => [
        'name' => 'reload',
        'options' => [
            [
                'name' => 'except',
                'shortcut' => 'e',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The commands to skip',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\RequestMakeCommand::class => [
        'name' => 'make:request',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the request',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the request already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ResourceMakeCommand::class => [
        'name' => 'make:resource',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the resource',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the resource already exists',
            ],
            [
                'name' => 'json-api',
                'shortcut' => 'j',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a JSON:API resource',
            ],
            [
                'name' => 'collection',
                'shortcut' => 'c',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a resource collection',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\RouteCacheCommand::class => [
        'name' => 'route:cache',
        'options' => [
            [
                'name' => 'dump-to',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Internal option used to dump fresh compiled routes to a temporary file',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\RouteClearCommand::class => [
        'name' => 'route:clear',
    ],
    \Hypervel\Foundation\Console\RouteListCommand::class => [
        'name' => 'route:list',
        'options' => [
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the route list as JSON',
            ],
            [
                'name' => 'method',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the routes by method',
            ],
            [
                'name' => 'action',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the routes by action',
            ],
            [
                'name' => 'name',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the routes by name',
            ],
            [
                'name' => 'domain',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the routes by domain',
            ],
            [
                'name' => 'middleware',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Filter the routes by middleware',
            ],
            [
                'name' => 'path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Only show routes matching the given path pattern',
            ],
            [
                'name' => 'except-path',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Do not display the routes matching the given path pattern',
            ],
            [
                'name' => 'reverse',
                'shortcut' => 'r',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Reverse the ordering of the routes',
            ],
            [
                'name' => 'sort',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'uri',
                'description' => 'The column (domain, method, uri, name, action, middleware, definition) to sort by',
            ],
            [
                'name' => 'except-vendor',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Do not display routes defined by vendor packages',
            ],
            [
                'name' => 'only-vendor',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Only display routes defined by vendor packages',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\RuleMakeCommand::class => [
        'name' => 'make:rule',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the rule',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the rule already exists',
            ],
            [
                'name' => 'implicit',
                'shortcut' => 'i',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate an implicit rule',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ScopeMakeCommand::class => [
        'name' => 'make:scope',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the scope',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the scope already exists',
            ],
        ],
    ],
    // REMOVED: Laravel's PHP development server; ServerStartCommandTest covers the native Swoole command.
    \Hypervel\Foundation\Console\StorageLinkCommand::class => [
        'name' => 'storage:link',
        'options' => [
            [
                'name' => 'relative',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the symbolic link using relative paths',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Recreate existing symbolic links',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\StorageUnlinkCommand::class => [
        'name' => 'storage:unlink',
    ],
    \Hypervel\Foundation\Console\StubPublishCommand::class => [
        'name' => 'stub:publish',
        'options' => [
            [
                'name' => 'existing',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Publish and overwrite only the files that have already been published',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite any existing files',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\TestMakeCommand::class => [
        'name' => 'make:test',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the test',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the test even if the test already exists',
            ],
            [
                'name' => 'unit',
                'shortcut' => 'u',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a unit test',
            ],
            [
                'name' => 'pest',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a Pest test',
            ],
            [
                'name' => 'phpunit',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create a PHPUnit test',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\TraitMakeCommand::class => [
        'name' => 'make:trait',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the trait',
            ],
        ],
        'options' => [
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the trait even if the trait already exists',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\UpCommand::class => [
        'name' => 'up',
    ],
    \Hypervel\Foundation\Console\VendorPublishCommand::class => [
        'name' => 'vendor:publish',
        'options' => [
            [
                'name' => 'existing',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Publish and overwrite only the files that have already been published',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Overwrite any existing files',
            ],
            [
                'name' => 'all',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Publish assets for all service providers without prompt',
            ],
            [
                'name' => 'provider',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The service provider that has assets you want to publish',
            ],
            [
                'name' => 'tag',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'One or many tags that have assets you want to publish',
            ],
        ],
    ],
    \Hypervel\Foundation\Console\ViewCacheCommand::class => [
        'name' => 'view:cache',
    ],
    \Hypervel\Foundation\Console\ViewClearCommand::class => [
        'name' => 'view:clear',
    ],
    \Hypervel\Foundation\Console\ViewMakeCommand::class => [
        'name' => 'make:view',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the view',
            ],
        ],
        'options' => [
            [
                'name' => 'extension',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'blade.php',
                'description' => 'The extension of the generated view',
            ],
            [
                'name' => 'force',
                'shortcut' => 'f',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the view even if the view already exists',
            ],
        ],
    ],
    \Hypervel\Notifications\Console\NotificationTableCommand::class => [
        'name' => 'make:notifications-table',
        'aliases' => [
            'notifications:table',
        ],
    ],
    \Hypervel\Queue\Console\BatchesTableCommand::class => [
        'name' => 'make:queue-batches-table',
        'aliases' => [
            'queue:batches-table',
        ],
    ],
    \Hypervel\Queue\Console\ClearCommand::class => [
        'name' => 'queue:clear',
        'arguments' => [
            [
                'name' => 'connection',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the queue connection to clear',
            ],
        ],
        'options' => [
            [
                'name' => 'queue',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The names of the queues to clear',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the operation to run when in production',
            ],
        ],
    ],
    \Hypervel\Queue\Console\FailedTableCommand::class => [
        'name' => 'make:queue-failed-table',
        'aliases' => [
            'queue:failed-table',
        ],
    ],
    \Hypervel\Queue\Console\FlushFailedCommand::class => [
        'name' => 'queue:flush',
        'options' => [
            [
                'name' => 'hours',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of hours to retain failed job data',
            ],
        ],
    ],
    \Hypervel\Queue\Console\ForgetFailedCommand::class => [
        'name' => 'queue:forget',
        'arguments' => [
            [
                'name' => 'id',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The ID of the failed job',
            ],
        ],
    ],
    \Hypervel\Queue\Console\ListFailedCommand::class => [
        'name' => 'queue:failed',
        'options' => [
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the failed jobs as JSON',
            ],
        ],
    ],
    \Hypervel\Queue\Console\ListenCommand::class => [
        'name' => 'queue:listen',
        'arguments' => [
            [
                'name' => 'connection',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of connection',
            ],
        ],
        'options' => [
            [
                'name' => 'name',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'default',
                'description' => 'The name of the worker',
            ],
            [
                'name' => 'delay',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of seconds to delay failed jobs (Deprecated)',
            ],
            [
                'name' => 'backoff',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of seconds to wait before retrying a job that encountered an uncaught exception',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the worker to run even in maintenance mode',
            ],
            [
                'name' => 'memory',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '128',
                'description' => 'The memory limit in megabytes',
            ],
            [
                'name' => 'queue',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The queue to listen on',
            ],
            [
                'name' => 'sleep',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '3',
                'description' => 'The number of seconds to sleep when no job is available',
            ],
            [
                'name' => 'rest',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of seconds to rest between jobs',
            ],
            [
                'name' => 'timeout',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '60',
                'description' => 'The number of seconds a child process can run',
            ],
            [
                'name' => 'tries',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '1',
                'description' => 'The number of times to attempt a job before logging it failed',
            ],
        ],
    ],
    \Hypervel\Queue\Console\MonitorCommand::class => [
        'name' => 'queue:monitor',
        'arguments' => [
            [
                'name' => 'queues',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The names of the queues to monitor',
            ],
        ],
        'options' => [
            [
                'name' => 'max',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '1000',
                'description' => 'The maximum number of jobs that can be on the queue before an event is dispatched',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the queue size as JSON',
            ],
        ],
    ],
    \Hypervel\Queue\Console\PauseCommand::class => [
        'name' => 'queue:pause',
        'arguments' => [
            [
                'name' => 'queue',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the queue to pause',
            ],
        ],
        'options' => [
            [
                'name' => 'all',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Pause job processing for all queues on all connections',
            ],
        ],
    ],
    \Hypervel\Queue\Console\PruneBatchesCommand::class => [
        'name' => 'queue:prune-batches',
        'options' => [
            [
                'name' => 'hours',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '24',
                'description' => 'The number of hours to retain batch data',
            ],
            [
                'name' => 'unfinished',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of hours to retain unfinished batch data',
            ],
            [
                'name' => 'cancelled',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of hours to retain cancelled batch data',
            ],
        ],
    ],
    \Hypervel\Queue\Console\PruneFailedJobsCommand::class => [
        'name' => 'queue:prune-failed',
        'options' => [
            [
                'name' => 'hours',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '24',
                'description' => 'The number of hours to retain failed jobs data',
            ],
        ],
    ],
    \Hypervel\Queue\Console\RestartCommand::class => [
        'name' => 'queue:restart',
    ],
    \Hypervel\Queue\Console\ResumeCommand::class => [
        'name' => 'queue:resume',
        'aliases' => [
            'queue:continue',
        ],
        'arguments' => [
            [
                'name' => 'queue',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the queue that should resume processing',
            ],
        ],
        'options' => [
            [
                'name' => 'all',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Resume job processing for all queues on all connections',
            ],
        ],
    ],
    \Hypervel\Queue\Console\RetryBatchCommand::class => [
        'name' => 'queue:retry-batch',
        'dispatcherOptionOffset' => 0,
        'arguments' => [
            [
                'name' => 'id',
                'mode' => 'optional',
                'isArray' => true,
                'default' => [],
                'description' => 'The ID of the batch whose failed jobs should be retried',
            ],
        ],
        'options' => [
            [
                'name' => 'isolated',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => false,
                'description' => 'Do not run the command if another instance of the command is already running',
            ],
        ],
    ],
    \Hypervel\Queue\Console\RetryCommand::class => [
        'name' => 'queue:retry',
        'arguments' => [
            [
                'name' => 'id',
                'mode' => 'optional',
                'isArray' => true,
                'default' => [],
                'description' => 'The ID of the failed job or "all" to retry all jobs',
            ],
        ],
        'options' => [
            [
                'name' => 'queue',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Retry all of the failed jobs for the specified queue',
            ],
            [
                'name' => 'range',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => true,
                'acceptValue' => true,
                'default' => [],
                'description' => 'Range of job IDs (numeric) to be retried (e.g. 1-5)',
            ],
        ],
    ],
    \Hypervel\Queue\Console\TableCommand::class => [
        'name' => 'make:queue-table',
        'aliases' => [
            'queue:table',
        ],
    ],
    \Hypervel\Queue\Console\WorkCommand::class => [
        'name' => 'queue:work',
        'arguments' => [
            [
                'name' => 'connection',
                'mode' => 'optional',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the queue connection to work',
            ],
        ],
        'options' => [
            [
                'name' => 'name',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => 'default',
                'description' => 'The name of the worker',
            ],
            [
                'name' => 'queue',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The names of the queues to work',
            ],
            [
                'name' => 'daemon',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Run the worker in daemon mode (Deprecated)',
            ],
            [
                'name' => 'once',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Only process the next job on the queue',
            ],
            [
                'name' => 'concurrency',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'The number of jobs to process at once',
            ],
            [
                'name' => 'stop-when-empty',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Stop when the queue is empty',
            ],
            [
                'name' => 'stop-when-empty-for',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'Stop when no jobs have been processed for the given number of seconds',
            ],
            [
                'name' => 'delay',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of seconds to delay failed jobs (Deprecated)',
            ],
            [
                'name' => 'backoff',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of seconds to wait before retrying a job that encountered an uncaught exception',
            ],
            [
                'name' => 'max-jobs',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of jobs to process before stopping',
            ],
            [
                'name' => 'max-time',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The maximum number of seconds the worker should run',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Force the worker to run even in maintenance mode',
            ],
            [
                'name' => 'memory',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '128',
                'description' => 'The memory limit in megabytes',
            ],
            [
                'name' => 'sleep',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '3',
                'description' => 'The number of seconds to sleep when no job is available',
            ],
            [
                'name' => 'rest',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '0',
                'description' => 'The number of seconds to rest between jobs',
            ],
            [
                'name' => 'timeout',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '60',
                'description' => 'The number of seconds a child process can run',
            ],
            [
                'name' => 'monitor-interval',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '1',
                'description' => 'The time interval of seconds for monitoring timeout jobs',
            ],
            [
                'name' => 'tries',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => '1',
                'description' => 'The number of times to attempt a job before logging it failed',
            ],
            [
                'name' => 'json',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Output the queue worker information as JSON',
            ],
        ],
    ],
    \Hypervel\Routing\Console\ControllerMakeCommand::class => [
        'name' => 'make:controller',
        'dispatcherOptionOffset' => 0,
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the controller',
            ],
        ],
        'options' => [
            [
                'name' => 'api',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Exclude the create and edit methods from the controller',
            ],
            [
                'name' => 'type',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => true,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Manually specify the controller stub file to use',
            ],
            [
                'name' => 'force',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Create the class even if the controller already exists',
            ],
            [
                'name' => 'invokable',
                'shortcut' => 'i',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a single method, invokable controller class',
            ],
            [
                'name' => 'model',
                'shortcut' => 'm',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Generate a resource controller for the given model',
            ],
            [
                'name' => 'parent',
                'shortcut' => 'p',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => true,
                'isArray' => false,
                'acceptValue' => true,
                'default' => null,
                'description' => 'Generate a nested resource controller class',
            ],
            [
                'name' => 'resource',
                'shortcut' => 'r',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a resource controller class',
            ],
            [
                'name' => 'requests',
                'shortcut' => 'R',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate FormRequest classes for store and update',
            ],
            [
                'name' => 'singleton',
                'shortcut' => 's',
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Generate a singleton resource controller class',
            ],
            [
                'name' => 'creatable',
                'shortcut' => null,
                'negatable' => false,
                'valueRequired' => false,
                'valueOptional' => false,
                'isArray' => false,
                'acceptValue' => false,
                'default' => false,
                'description' => 'Indicate that a singleton resource should be creatable',
            ],
        ],
    ],
    \Hypervel\Routing\Console\MiddlewareMakeCommand::class => [
        'name' => 'make:middleware',
        'arguments' => [
            [
                'name' => 'name',
                'mode' => 'required',
                'isArray' => false,
                'default' => null,
                'description' => 'The name of the middleware',
            ],
        ],
    ],
    \Hypervel\Session\Console\SessionTableCommand::class => [
        'name' => 'make:session-table',
        'aliases' => [
            'session:table',
        ],
    ],
];
