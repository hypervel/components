...
namespace Hypervel\Tests\Routing\RoutingRouteTest;
...
class RoutingRouteTest extends TestCase
{
    ...
    public function testBasicDispatchingOfRoutes()
    {
        ...
        $router->query('foo/bar', function (): string {
            return 'query hello';
        });
        $this->assertSame('hello', $router->dispatch(Request::create('foo/bar', 'GET'))->getContent());
    ...
    public function testImplicitBindingsWithOptionalParameterUsingEnumIsAlwaysCastedToEnum()
    {
        ...
        $router->get('foo/{bar?}', [
            'middleware' => SubstituteBindings::class,
            'uses' => function (?\Hypervel\Tests\Routing\Fixtures\CategoryBackedEnum $bar = null) {
    ...
    public function testRouteCanMiddlewareCanBeAssigned()
    {
        ...
        $this->assertEquals([
            'foo',
            'can:create,Hypervel\Routing\Route',
        ], $route->middleware());
...
// Model stubs override resolveRouteBinding() directly instead of hooking into
// where()/first() like Laravel's stubs do. In Laravel (no strict_types), the
// inherited resolveRouteBinding() chain works because resolveRouteBindingQuery()
// can return a Model from a Builder|Relation typed method without error. In
// Hypervel (strict_types on Model.php), that return type is enforced at runtime.
// Overriding resolveRouteBinding() directly is the simplest fix — these stubs
// are faking DB lookups anyway, and routing code only calls resolveRouteBinding(),
// never where()/first() directly.
...
// See comment on RouteModelBindingStub for why resolveRouteBinding() is overridden.
...
// See comment on RouteModelBindingStub for why resolveRouteBinding() is overridden.
// resolveChildRouteBinding() is overridden for the same reason — the base Model's
// version goes through resolveChildRouteBindingQuery() → resolveRouteBindingQuery()
// which has the same strict Builder|Relation return type issue.
class RoutingTestUserModel extends Model
{
    ...
    {
        // Must use childRouteBindingRelationshipName() to pluralize/camelCase
        // the child type (e.g., 'post' → 'posts') — matching the base Model's
        // resolveChildRouteBindingQuery() behavior.
        $related = $this->{$this->childRouteBindingRelationshipName($childType)}();
        $related->value = $value;
...
class RoutingTestTeamModel extends Model
{
    ...
    public function resolveChildRouteBinding(string $childType, mixed $value, ?string $field): mixed
    {
        // See RoutingTestUserModel::resolveChildRouteBinding for why this uses
...
{
    // Inherits resolveChildRouteBinding() from RoutingTestTeamModel which calls
    // $this->users() via childRouteBindingRelationshipName(). This override makes
    // that throw, simulating a relationship that doesn't resolve the child model.
    public function users()
    {
