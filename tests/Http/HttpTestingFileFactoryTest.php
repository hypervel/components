...
class HttpTestingFileFactoryTest extends TestCase
{
    ...
    public function testCreateWithMimeType(): void
    {
        $this->assertSame(
            'audio/webm',
    ...
    public function testCreateWithoutMimeType(): void
    {
        $this->assertSame(
            'video/webm',
    ...
    public function testExplicitZeroSizeOverridesFileContentSize(): void
    {
        $file = (new FileFactory)->createWithContent('document.txt', 'content')->size(0);

        $this->assertSame(0, $file->getSize());
    }

    #[DataProvider('generateImageDataProvider')]
    public function testCallingCreateWithoutGDLoadedThrowsAnException(string $fileExtension, string $driver): void
    {
    ...
    /**
     * Determine if GD supports the given driver.
     */
