...
// tap function is globally available from helpers.php
...
class LazyCollectionTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        // Test basic chunking based on consecutive values
        $collection = LazyCollection::make([1, 2, 3, 10, 11, 12, 20, 21]);

        $chunks = $collection->chunkWhile(function ($value, $key, $chunk) {
            return $chunk->last() + 1 === $value;
        });

        $result = $chunks->map(function ($chunk) {
        ...
        // Test with keys preservation
        $collection = LazyCollection::make(['a' => 1, 'b' => 2, 'c' => 10, 'd' => 11]);
        ...
        $this->assertSame([], $chunks->all());

        // Test single element
        $collection = LazyCollection::make([42]);
