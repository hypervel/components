...
- Resolve the production dependency cycle between `hypervel/foundation` and `hypervel/testing`, which currently causes test-only classes and PHPUnit integration to ship with every Foundation install. Decide whether Foundation's testing namespace moves to `hypervel/testing` or Foundation changes its dependency to development-only while preserving split-package installation and public namespaces.
