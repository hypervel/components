...
Implicit route binding stays string-backed only. `RouteSignatureParameters` filters route parameters through `Reflector::isParameterBackedEnumWithStringBackingType()`, matching Laravel and both documentation sets. Widening that contract would also let paths such as `/1`, `/01`, and `/%201` alias one enum case while `route()` generates only `/1`, so Routing keeps its existing string-specific conversion rather than using the generic helpers below.
...
- Cast definitions are evaluated once per extraction. An empty declaration returns the validator result immediately. An extraction with only ordinary exact paths skips placeholder initialization and whole-tree key encoding; wildcard paths, escaped literal paths, and dotted declarations that collide with an exact top-level input key use the encoded pipeline for the whole extraction. Casters receive the original decoded validated array, not partially cast output, so sibling access is stable and declaration order does not change a caster's context.
...
- Before construction, require a declared class to implement `CastsRequestInput` or `RequestCastable`; reject existing non-caster classes with `InvalidCastException` without running their constructors. A class-string returned by `RequestCastable` remains responsible for satisfying its documented contract and otherwise fails natively.
...
| `decimal:<scale>` | Decimal string produced by `BigDecimal::of((string) $value)->toScale($scale, RoundingMode::HalfUp)`; the scale must be a non-negative integer, and Brick's value-conversion failures are translated to `Hypervel\Support\Exceptions\MathException` as Eloquent does |
...
Validate decimal declaration scales at their two owning boundaries. Foundation checks a present decimal declaration once before casting its matches and throws `InvalidArgumentException` naming the input when the argument is missing, empty, non-numeric, fractional, negative, or outside PHP's integer range. Missing validated input skips its declaration with every other request cast, avoiding unused work. Eloquent applies the same rule in `HasAttributes::getCastType()` after normalization identifies `decimal` and before either its per-model or worker-lifetime cast-type cache is populated; its exception names the attribute. The Eloquent placement covers both bare `decimal` and `decimal:<scale>`, validates only on a cache miss, ensures a failed declaration cannot poison either cache, and correctly exposes invalid model configuration even when the attribute is absent. Use `filter_var(..., FILTER_VALIDATE_INT, ['options' => ['min_range' => 0]])`; do not invent an upper bound. Keep local `ensureValidDecimalScale()` helpers because the two packages have different error vocabularies and sharing this small guard would add cross-package coupling without removing meaningful complexity.
...
4. For an integer-backed enum, accept integers and booleans, convert trimmed non-empty numeric strings with PHP's own numeric-string conversion, and accept floats only when finite and inside the platform integer range, matching PHP's coercive integer-parameter conversion. Reject non-finite and out-of-range values such as `'1e999'` and `'9223372036854775808'` as `null`; in-range fractional values retain PHP's intentionally lossy conversion (`1.9` to `1`).
...
`enum_from()` delegates to `enum_try_from()`. If no case is returned, it throws one `ValueError` with native-style invalid-backing-value wording that includes both the rejected value and target enum class. Strings are quoted, scalar values use `var_export()`, and non-scalars use their bounded `get_debug_type()` name so request arrays cannot produce unbounded exception messages. The throwing helper is for construction paths; validation and conditional lookup use the non-throwing helper directly, so invalid input never pays an exception-driven-control-flow cost.

Use the helpers consistently at every generic dynamic backed-enum conversion site:

- Validation's `Rules\Enum` uses `enum_try_from()`.
...
3. Keep all six Laravel-compatible `Validator` wrappers/extension points, but make their placeholder work thin `ValidationData` delegates: public `parseData()`; protected `replacePlaceholders()`, `replacePlaceholderInString()`, and `replaceDotPlaceholderInParameters()`; and protected static `encodeAttributeWithPlaceholder()` and `decodeAttributeWithPlaceholder()`. Preserve Hypervel's existing literal-asterisk handling in addition to Laravel's literal-dot behavior. The hash and algorithms have one owner.
   `replaceDotPlaceholderInParameters()` stringifies raw scalar parameters because inline date-comparison failures reach `addFailure()` directly through `PlanExecutor`, unlike delegated rules whose parameters were already normalized by `validateAttribute()`.
...
FormRequest first checks whether placeholder space is required. Any declaration containing `*` (including an escaped literal asterisk), an escaped literal dot, or an ordinary dotted path that also exists as an exact top-level input key selects the encoded pipeline for the whole extraction. The last condition prevents `Arr::get()` from reading the literal key while `Arr::set()` writes a new nested path. A direct `foreach` with an early `break` performs this declaration check without copying the cast keys.

Ordinary exact paths otherwise read and write the decoded validated array directly. This avoids two recursive copies and placeholder initialization when nested/wildcard behavior is not used. Do not add path-local encoding or merge encoded and decoded outputs.

The encoded pipeline keeps placeholder space internal:
...
- Add `Hypervel\Data\Concerns\RequestCastableData`, parallel to `EloquentCastableData`. Its single method returns a fresh `Hypervel\Data\Http\DataRequestCast` for `static::class`. Direct Data, Dto, and Resource request casts accept no declaration arguments and reject any supplied arguments with a class-specific `InvalidArgumentException` rather than silently ignoring Eloquent-only options.
...
- Keep `Hypervel\Data\Http\DataRequestCast` at the HTTP integration root, parallel to the internal Eloquent adapters. `Http\Casts` contains the public `As...` declaration helper, not every internal class that performs conversion.
...
- `enum()`, `enums()`, and `whenEnum()` delegate to `enum_try_from()`. This makes them idempotent on matching cast values while sharing the same strict scalar normalization with Validation, Collections, Foundation, Database, and Data.

Do not add a Laravel-difference note: this is a correctness fix that preserves the helper contracts.

## Documentation Result

Edit only the existing canonical sections in `src/docs/validation.md`, `src/docs/data-objects.md`, and `src/docs/eloquent-mutators.md`; do not rewrite these files.
...
- casts running fresh for every `validated()` / `safe()` extraction, with no cached result objects;
- guidance to pair casts with validation rules that guarantee a compatible raw shape because failed post-validation conversion throws;
...
- Eloquent's existing decimal cast documentation stating that its precision must be a non-negative integer;
...
3. **Extract validation placeholder and wildcard ownership.**
   ...
   - Cover a failing inlined date-comparison rule with an integer parameter, proving placeholder decoding avoids a strict-types `TypeError` and exposes the same string parameter shape as delegated execution.
...
4. **Rebuild Foundation casting.**
   ...
   - Reject `AsEnumCollection::class` without its required enum argument through a clear `InvalidArgumentException` instead of an undefined-offset warning.
   - Validate every present `decimal` declaration before casting its matches. Accept scales such as `0` and `2`; reject bare, empty, non-numeric, fractional, negative, multi-argument, and out-of-range scales with an input-specific `InvalidArgumentException`, leaving value-conversion failures on the existing math-exception path.
   ...
   - Move `tests/Foundation/Http/CustomCastingTest.php` to `tests/Foundation/Http/FormRequestCastingTest.php`, use a test-specific namespace for inline helpers, and replace old-API assertions with this public-behavior matrix:
     ...
     - Valid decimal scales `0` and `2`, plus clear declaration failures for bare, empty, non-numeric, fractional, negative, multi-argument, and out-of-range scales.
     ...
     - An explicit date-time format mismatch raises Carbon's `InvalidFormatException` without falling back to generic parsing.
     - Deep and associative wildcards, partial-segment wildcards, exact escaped dotted keys, literal asterisk keys, and a dotted child key beneath a wildcard whose output key must remain literal. Also prove that an unescaped dotted cast declaration does not consume a top-level literal-dot key accepted by an escaped validation rule or create a new nested key; this guards the exact-path optimization's `Arr::get()` / `Arr::set()` collision boundary.
     ...
     - An existing non-caster declaration is rejected before its constructor runs.
   ...
   - Update Eloquent's `src/database/src/Eloquent/Concerns/HasAttributes.php` to validate the raw decimal declaration after `getCastType()` normalizes it to `decimal` and before writing either cast-type cache. Keep the decimal cast arm unchanged. Add matching valid and invalid declaration coverage to `tests/Integration/Database/EloquentModelDecimalCastingTest.php`, then run that file immediately on the configured database test environment. In the ported missing-primitive-attribute fixture, map the internal normalized `decimal` marker to a valid `decimal:2` declaration and explain the coupling with a short comment; keep every assertion and primitive entry so coverage is not weakened.
...
5. **Integrate Data objects.**
   ...
   - Prove direct Data/Dto/Resource declarations reject positional arguments rather than silently accepting Eloquent-only cast options.
...
7. **Rewrite the canonical documentation sections.**
   ...
   - Update the decimal cast entry in `src/docs/eloquent-mutators.md` with its non-negative integer precision requirement.
   - Search the documents for stale API names after each targeted edit.

8. **Focused and full verification.**
   - Re-run the changed Foundation, Data, Support enum/collection/data-helper, Validation enum/ValidationData/wildcard, Database enum integration, cleanup-subscriber, and package-metadata tests together from the worktree root.
...
- Integer-backed numeric strings work consistently through Validation, Collections, FormRequest, Eloquent JSON/scalar casts, Data casts/morphs, and typed `ValidatedInput`; invalid conditional lookups remain exception-free and throwing construction paths retain useful, bounded `ValueError` failures.
...
- Old request-casting contracts, helpers, files, tests, docs, and empty directories are gone; Eloquent retains its existing public casting API while rejecting malformed decimal declarations before caching them.
