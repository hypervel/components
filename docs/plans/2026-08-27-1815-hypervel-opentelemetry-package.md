...
- Export OTLP over HTTP using protobuf by default. JSON is selectable. An OTLP/gRPC exporter, Prometheus scrape endpoint, Influx line protocol, StatsD output, and Telegraf-specific integration are permanently excluded: a Collector, relay, or backend owns protocol translation and fan-out.
...
- Keep `ext-protobuf` as a package suggestion, not a hard requirement; `google/protobuf` remains the portable implementation.
...
Reject `grpc` during configuration normalization with a direct unsupported-protocol error; accepted protocols are `http/protobuf` and `http/json`. The explicit client is Guzzle's PSR-18 implementation. Hypervel's server and ordinary command coroutine runtimes default to `SWOOLE_HOOK_ALL`, which includes native-cURL, TCP, and sleep hooks, so transport sends and retry waits yield from the scheduler coroutine. Prove that assumption with a slow-endpoint process test; do not infer non-blocking behavior merely from the existence of a coroutine.
...
- Store in-flight producer state in one `QueueProducerStateStore implements NonCopyableContext` in `CoroutineContext`: a primary framework-UUID map, an exact final-payload-to-UUID index, and separate exact-payload timing state for trace-disabled producer metrics. Database/SQS batches can retain multiple entries before any terminal event, so a mutable store avoids array copy-on-write and `NonCopyableContext` prevents a forked child from mutating its parent's entries. Normal `JobQueued` / `JobQueueingFailed` correlation uses the exact-string index without JSON decoding; computing its first string hash is proportional to payload length but remains cheaper and lower-allocation than decoding. If a later finalizer listener re-encodes the mutable event payload after OTel's listener, decode that final terminal payload once and resolve the primary state by its preserved UUID. Do not depend on `traceparent` for private correlation because propagation may omit trace context. Removing a state also removes its original exact index. Trace-disabled producer metrics use the same finalizer, create no span, decode only when propagation independently requires it, and retain timing under the exact final payload. The package never listens to `JobQueueing`.
- A later listener that removes/corrupts the framework UUID or throws after OTel's finalizer has broken the queue payload lifecycle and supplies no exact terminal correlation fact. In a Database/SQS batch, a later message's finalizer failure can likewise leave earlier finalized sibling states without terminal events because the framework has not begun the batch attempt. Coroutine teardown or process exit releases these orphans; a long-running non-coroutine custom process that catches and repeatedly continues such broken dispatches retains one producer state, span context, and payload-sized exact-index key per orphan until it exits. Do not guess among concurrent batch states, add a second private correlation field, sweep, cap, or listener-order mechanism.
...
- OTLP shared/per-signal precedence for endpoints, headers, protocol/content type, compression, timeouts, CA certificate, client certificate, and client key; shared-endpoint signal-path appending versus verbatim signal endpoints; readable-file and client-certificate/key-pair validation; Guzzle `verify`/`cert`/`ssl_key` mapping; explicit shared Guzzle request/stream/URI factory wiring without PSR discovery; compatible-signal client reuse, incompatible timeout/TLS tuple separation; and the upstream retry limit;
...
- one tick, smallest cadence, per-signal due times, monotonic advancement, periodic/manual no-overlap, internal `true`/`false`/`null` completed/failed/skipped results, public manual contention mapped to `false`, periodic contention skipped without diagnostics, and closing waiting for an in-flight flush before provider shutdown;
...
- slow local OTLP endpoint proves native-cURL/retry sleeps yield: a concurrent heartbeat/request coroutine continues while export is pending;
...
- a config-cache fixture freezes SDK-resolved `OTEL_*` values at cache-build time until rebuild, matching the documented deployment behavior;
