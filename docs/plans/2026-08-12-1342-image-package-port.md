...
The implementation is complete. The canonical CI images must include working Imagick AVIF and HEIC encoders, pass their real-encode smoke checks, and then pass the expanded PHP 8.4/8.5 consumer workflows. Make the package public once the organization owner has permission to change its visibility.
...
| CI's GD build lacks WebP/AVIF and installs no Imagick, leaving the headline paths failing or permanently skipped. PECL also rewrites Imagick's shipped arginfo headers before their stub sources, so `make install` needlessly regenerates them and depends on a second network download of PHP-Parser that can fail after the extension has compiled. Installing the same toolchain and extensions in every matrix job also duplicates slow work and makes one transient release-host failure fan out across the suite. Debian's recommended libheif encoder plugins are also omitted by `--no-install-recommends`, so ImageMagick recognizes AVIF/HEIC but cannot encode either format. | Build focused PHP 8.4 and 8.5 CI images once a week from the Swoole 6.2.2 base. Build GD with both codecs, install `libheif-plugin-aomenc` and `libheif-plugin-x265`, and use checksum-verified PIE 1.4.9 for the latest compatible stable `phpredis/phpredis` and `imagick/imagick` releases. Validate every required extension, then perform real Imagick AVIF and HEIC encodes before publishing. Run all PHP jobs from those images. |
...
- [x] PHP 8.5 compatibility and test corrections — let native floats bypass Eloquent's string sentinel matching; key in-memory refresh state by the resolved connection name while retaining nullable default connection resolution; query the current locale with string `'0'`; resolve closure `self`, `parent`, and `static` return types to their concrete scope/called classes; validate gRPC timeout overflow before integer conversion; and stably sort each facade proxy's merged native/documented methods by case-insensitive name so reflection-order changes do not rewrite generated inventories. Update Any-mode cache mocks to expect the production `HSETEX` command rather than the superseded `HSET`/`HEXPIRE` pair.
...
- [x] `tests/Image/ImageTest.php` — copy upstream and preserve its public API breadth, then correct/add:
  ...
  - AVIF extension detection reads `tests/Image/Fixtures/image.avif`, verifies its `ftypavif` brand and parseable dimensions independently of libmagic, and asserts the mapped `avif` extension when fileinfo recognizes `image/avif`; it does not depend on Imagick or an encoder. Regenerate the one-pixel red fixture with `php -r '$image = imagecreatetruecolor(1, 1); $red = imagecolorallocate($image, 255, 0, 0); imagefill($image, 0, 0, $red); imageavif($image, "tests/Image/Fixtures/image.avif");'`;
