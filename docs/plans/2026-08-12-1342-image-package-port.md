...
| CI's GD build lacks WebP/AVIF and installs no Imagick, leaving the headline paths failing or permanently skipped. | Install both GD codecs and pinned Imagick 3.8.1 in the shared PHP setup action. |
...
- [ ] `.github/actions/setup-php/action.yml` — add `libwebp-dev`, `libavif-dev`, and `libmagickwand-dev`; configure GD with `--with-webp --with-avif`; install and enable `imagick-3.8.1` only when absent. Preserve the existing conditional extension setup, then run a PHP assertion that fails setup unless `imagewebp()`, `imageavif()`, and the Imagick extension are available. This prevents a future base image's incomplete preinstalled GD from bypassing configuration and silently skipping codec tests.
...
- CI executes GD WebP/AVIF and Imagick paths rather than silently skipping them.
