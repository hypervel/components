...
- Saloon's `BaseResource` is not included. A normal resource class with a constructor-promoted concrete connector retains integration-specific methods and DTO typing, while the upstream base class narrows both to the abstract Connector.
...
Request remains caller-owned and mutable while preparing one operation. It implements `SelfBuilding`, and the abstract base declares `public static function newInstance(): static` returning `new static`, so an unbound no-argument or default-argument request resolves freshly instead of entering Hypervel's auto-singleton cache. Requests with required caller-supplied constructor arguments use direct construction or Saloon's argument-forwarding `make(...$arguments)` API; an application that needs container-defined construction uses an explicit transient binding. Container parameters are not forwarded to `newInstance()`, so bare and parameterized container resolution both fail naturally for a required-argument request. Do not add reflection or constructor-forwarding machinery. Do not document constructor injection of mutable Request objects into worker-shared services. PendingRequest, Response, middleware repositories, and body repositories are operation-owned and must never be stored on a worker singleton.

`Request::__clone()` must clone every initialized mutable repository owned by the request: headers, query, options, delay, middleware pipeline, and body. `MiddlewarePipeline::__clone()` must in turn clone its internal pipelines. Cookie and cache-control arrays/scalars copy with the request, while the final readonly RetryPolicy may be shared until a fluent call replaces it. Do not recursively clone caller-supplied authenticators, objects, or stream resources stored inside those repositories, and do not clone the explicitly shared MockClient used to record pooled work. This makes paginator-created request clones independent without inventing a general object-graph copier.
...
Match Hypervel HTTP's `withCookies(array $cookies, string $domain)` signature exactly. Retain each cookie group with its caller-supplied domain in the operation-owned repository, then have Sender replay each group through Hypervel HTTP's dedicated cookie method. Do not infer a domain from the final URI, since callers may intentionally target a parent domain. Cache identity continues to export the effective cookie jar's stable fields.
...
For each attempt, the manager finalizes the operation before the sender runs. The sender must:

1. receive the manager's resolved transport snapshot containing the fixed connection name, validated connection options, and validated request options;
2. call `Factory::createPendingRequest()` so framework global middleware, fakes, events, and test controls apply;
3. apply the final Saloon URI, headers, cookies, auth, TLS/certificate settings, and snapshotted Guzzle options;
4. read the snapshotted connection and operation Telescope tags, prepend `TelescopeTag::Saloon`, and set the combined tags through Hypervel HTTP while preserving `telescope_enabled: false`;
...
Each body trait owns its default solely through `defaultBodyRepository()`, which PendingRequest snapshots before plugin boot. JSON, form, and XML boot hooks only supply a missing content type; string and stream bodies need no boot hook, while PendingRequest assigns multipart's boundary-aware content type when it prepares that repository. Do not retain a second, unreachable body-initialization path in the boot hooks.
...
Reject non-string header names at both owning boundaries. Hypervel HTTP validates normal requests, while Saloon's normalizer validates logical fake/cache requests that deliberately bypass HTTP. A list-shaped header array must fail instead of becoming numeric header names.
...
Build the final URI once after ordinary request middleware and use it for the sender, direct fake/cache PSR construction, cache identity, events, and response request access. When joining a relative endpoint, retain the base URL's raw query before the endpoint's raw query so valid integration-wide parameters are not lost. Do not pass a separate Guzzle `query` option: it replaces the URI's existing query string.

Retain Saloon's repository-over-URI precedence without its lossy `strtok()` parser. Split the combined base and endpoint raw query into pairs and decode only each name with query-string (`urldecode`) semantics. For every top-level repository key, remove raw pairs whose decoded name equals that key or begins with that key followed by `[`, so a scalar, null, empty array, or nested value replaces the same query family from either URL. Retain all other raw pairs, then append `http_build_query($repository, '', '&', PHP_QUERY_RFC3986)`. This preserves untouched ordering, duplicate keys, valueless pairs, zero/false/empty-string names and values, and original encoding while retaining Saloon's top-level nested-array precedence. Attach the result through the PSR URI; do not add a general query parser or normalize untouched URL pairs.
...
Before authoritative capture, `PendingRequest::toPsrRequest()` returns an uncaptured logical snapshot without memoizing URI/body state, applying connector/request PSR hooks, notifying final-PSR observers, or setting retry offsets. After a fake, cache, or network path captures the authoritative request, it returns that request. This keeps signing/nonces and debug observers single-invocation while allowing a sending-event listener to inspect then continue customizing the operation. A caller that reads a user-owned non-seekable body from this low-level snapshot consumes those bytes; do not buffer or deep-copy arbitrary streams.
...
Hypervel HTTP's `withoutRedirecting()` stores `false`, while `withOptions()` may store either supported Guzzle boolean form, but `maxRedirects()` indexes into that value as an array. The `false` form triggers PHP's deprecated scalar-to-array conversion and the `true` form already fatals. Have `maxRedirects()` replace a non-array redirect value with an empty array before setting `max`, preserving array sibling settings and last-call-wins fluent behavior. Apply the same direct guard to Saloon's operation-owned options; do not add another redirect API or alter option-layer merging.
...
`multiplex` remains in request options for every mode; only `Multiplexing::NONE` also configures handler selection. `Factory::getConnectionOptions()` already removes `transport_sharing` from request options; retain that behavior. Add both cap keys to `ReservedOptions`' unconditional rejection set so registered connections, global options, fluent `withOptions()`, raw request options, and per-send overrides fail with guidance to use bounded coroutine fan-out or the rate limiter. Do not add a package-owned aggregate concurrency semaphore without a concrete API contract that requires one.
...
Hypervel HTTP currently passes every URL through `UriTemplate::expand()`, whose RFC 6570 behavior removes undefined brace expressions even when the application never enabled URL parameters. Make expansion opt-in by returning the original URL when `$urlParameters === []`; retain normal URI-template behavior once parameters are configured. Cover an unconfigured literal-brace URL, an encoded literal brace alongside a configured template, and unchanged ordinary expansion. Saloon's final PSR URI already encodes preserved raw query braces before Sender sees it; add a regression proving the encoded logical URI, cache identity, and transmitted request agree. Do not add a second Saloon URL-template path.
...
This is a protected extension seam for response subclasses, not a second exception pipeline. Saloon overrides `failed()` so a non-null request `hasRequestFailed()` verdict wins over the connector verdict and status remains the fallback; `successful()` stays the inherited status predicate. It also overrides `toException()` and `throw()` narrowly so `shouldThrowRequestException()` remains authoritative without a `throw null` path. Explicit-status helpers remain literal, while failure-named helpers honor the integration failure policy. Keep the existing OR rule for the non-nullable request/connector throw hooks instead of adding another tri-state mechanism.
...
Request middleware mutates the one operation-owned PendingRequest and may return a FakeResponse to short-circuit it; a returned PendingRequest has no meaning and is ignored. This avoids splitting URI/body preparation from the original operation's cache, rate-limit, event, response, and retry ownership. Keep runtime tolerance for fluent callbacks that return the request, but type closure middleware as `FakeResponse|void` and class middleware as `?FakeResponse`. Response middleware remains `Response|void` because replacing the response is supported and authoritative.
...
6. run ordinary connector, request, plugin, and worker-global request middleware, then dispatch `SendingSaloonRequest` for every operation when listeners exist;
7. finalize the URI, materialize the operation body once, validate caching, and resolve one local transport snapshot. Everything after this freeze point reads operation state rather than customizing it;
8. match an explicit fake before cache lookup and inspect the server cooldown before configured admission policies; skip admission, delay, and transport once a fake or cache hit supplies a response;
9. use `Sleep` for configured delay only when the network will be called;
10. call the sender once with the resolved transport snapshot or use the short-circuit response;
...
12. run response/fatal middleware, then resolve the current attempt's retry policy from PendingRequest at the retry decision and evaluate error handling. This makes request, connector-boot, request-middleware, response-middleware, and fatal-middleware retry customization authoritative without capturing an earlier snapshot.
...
Resolve and validate the transport snapshot before fake and cache lookup. A short-circuit send therefore still requires a valid registered connection and catches an invalid connector-selected connection in tests. The snapshot covers only the connection name and connection/request options; Sender reads the finalized URI, headers, cookies, and prepared body from the operation.
...
`Hypervel\Saloon\Http\Response` extends `Hypervel\Http\Client\Response`. Removing Saloon's conflicting repository-first response signatures lets it inherit the complete Hypervel HTTP surface directly: body and JSON decoding, headers, cookies, status predicates, transfer metadata, throwing, ArrayAccess, macros, PSR access, and string conversion. Add only Saloon-owned `request()`, `connector()`, `pendingRequest()`, and `toPsrRequest()` access plus failure-policy hooks, DTO, XML/DOM, export, debugging, and cached/mocked state. Do not forward or reimplement inherited methods.

Create it through `fromResponse(HypervelResponse $response, PendingRequest $pendingRequest, RequestInterface $request)`. Copy the framework response's PSR response, cookies, transfer stats, decoder, decoded state, and exception settings as `ApiResponse::createFrom()` already does, then attach Saloon's operation references. Custom response classes use the same factory with the current base Saloon response, so any body buffering and framework metadata survive class selection. Do not retain `fromPsrResponse()` or response-side sender-exception state; transport failures become `FatalRequestException` before a response exists.
...
Retain the `HasResponse` companion trait for DTOs implementing `WithResponse`. It stores the original response in an uninitialized typed property, matching PHP's normal fail-fast behavior before `setResponse()` is called, and prevents each DTO from repeating the same property and accessors.
...
Remove the curated status-specific exception classes. `Hypervel\Saloon\Exceptions\Request\RequestException` extends the framework RequestException and exposes typed `response()`, `status()`, and PendingRequest access. `ClientException` and `ServerException` retain useful 4xx/5xx catch boundaries, and `FatalRequestException` remains the transport-failure type. Request and connector custom exception hooks return this request-exception type; request wins over connector, and custom classes must extend it so the inherited response pipeline remains fully typed. Saloon Response overrides `newRequestException()` to select a custom instance or the three built-in response exception types. Forward `truncateExceptionsAt` when constructing built-in exceptions; a user-constructed custom exception owns its message settings. Framework `DeterminesStatusCode` predicates work on the exception through `status()`; unlisted codes remain available through the integer status rather than requiring one class per code.
...
Use immutable dates for held expiry values and capture every date modifier result. Accept only non-negative integer expiry durations whose addition to the captured current timestamp remains representable; reject values that would wrap the held date. Mark client secrets, access/refresh tokens, passwords, certificate passphrases, and token-exchange inputs as sensitive. Preserve Saloon's base-URL override guard so credentials cannot be redirected to arbitrary endpoints unless a connector, request, or OAuth config explicitly opts into an application-controlled absolute endpoint through its stable resolver/value; do not port the public mutable override flags.
...
Saloon's process-global `Config::preventStrayRequests()` is not retained. The framework-level `Http::preventStrayRequests()` guard already propagates through every pending HTTP request created by Sender and prevents an unmocked Saloon request from reaching the network. MockClient's own controls remain responsible only for unmatched requests while that client is active.
...
Store a fixture as typed status, headers, body, and context. Redact configured headers and JSON/body patterns before writing, and fail before publication when any configured regular expression cannot be compiled or executed. Use the framework `Filesystem` directly and its atomic `replace()` method after creating the parent directory; do not port Saloon's package-owned Storage wrapper. Because users construct fixtures directly, accept an optional Filesystem in the constructor and resolve the stateless concrete from the container only when none is supplied. Tests must use `ParallelTesting::tempDir()` and own cleanup.
...
Default cacheable methods are GET, HEAD, OPTIONS, and QUERY. RFC 10008 makes QUERY responses cacheable and requires request content in their key, which the prepared-body identity already provides. Cache successful responses only. Store one immutable payload containing status, headers, and body; the framework repository owns TTL and serialization, so do not duplicate expiry timestamps or cache-driver wrappers.

The default key includes connector class, request class, method, final logical URI, normalized headers, cookies, dedicated authentication/certificate state, prepared body bytes, and normalized transport options that can change the successful response (`allow_redirects`, certificates/keys, content decoding, `curl`, `expect`, IP-family/IDN selection, proxy, TLS verification/crypto method, and HTTP version), then hashes the canonical representation once with SHA-256. Keep `expect`: it changes the origin-visible `Expect` header, so an origin may return different successful bytes or metadata. Omit only timing, diagnostics, observation callbacks, and execution-shape controls that cannot change successful response bytes or metadata.

This key is an attacker-influenced cross-tenant response-isolation boundary: a forged collision could return one tenant's cached response to another. SHA-256 therefore follows the `AGENTS.md` exception for trust boundaries. Seeded `xxh128` remains appropriate for internal table keys, but it does not provide a cryptographic collision-resistance contract and would add stable-secret coordination to every shared cache store. Lowercase and sort header names while preserving each header's value order; sort other associative maps recursively without reordering lists. Export cookie jars to stable cookie fields and reject any default identity value that cannot be represented safely. Only a package prefix and hash enter the backend. A custom `cacheKey()` replaces the canonical identity material but is still hashed, keeping backend keys bounded and preventing raw secrets from becoming key names. For a seekable prepared stream, hash from its current logical contents and restore the original position. Require a custom key for a non-seekable body rather than consuming it.
...
Use Laravel-style public paginator names: `totalResults()`, `maxPages()`, `perPageLimit()`, `request()`, `currentPage()`, and `startPage()`. Keep recognizable upstream `get*` names only on protected subclass extension hooks.
...
Add `Hypervel\Saloon\RateLimit\Queue\ReleaseOnRateLimit` for queued integration work. It runs the job, catches only `RateLimitReachedException`, and releases the job for `result()->retryAfter()` seconds without rethrowing the handled backoff. Other exceptions propagate unchanged. The middleware uses the queue job's existing `release()` method and imports no queue classes, so it adds no package dependency or alternate limiter path. Each release still consumes a queue attempt.
...
Reuse existing numeric state columns; cooldown needs only `expires_at`, so no migration change is required. Add full store contract coverage, key stability, overflow validation, concurrent extension, inspection, expiry, clear, pruning, and fake-time tests. Update `src/docs/rate-limiting.md`.

For resources using `HasRateLimits`, inspect the transport response before user response middleware, then parse `Retry-After` as non-negative delta seconds or any of the three RFC 9110 HTTP-date forms with one captured immutable GMT clock value. Validate the weekday token but parse the remaining timestamp without it so PHP cannot shift a mismatched date. Apply RFC 850's 50-year rule against the complete parsed timestamp, normalize the allowed leap second before parsing, and reject parser warnings or errors. A valid, representable future delay imposes the cooldown even if later middleware throws. Return the original 429 through normal response/error handling; cooldown detection itself must not recursively resend it. A later operation—or a new attempt explicitly requested by the normal retry policy—sees the shared cooldown and throws or waits according to the resource policy. Missing, malformed, unrepresentable, zero, or already elapsed delays do not create state. Do not invent a package-wide maximum; integrations may clamp the protected resolver when their provider defines one.
...
All generator commands extend Hypervel's `GeneratorCommand` and retain its standard `--target-path` and `--target-namespace` options. An explicit command option takes precedence over Saloon config for that invocation; package config remains the default when the corresponding option is absent.
...
- cache duration/store/key/invalidation behavior, QUERY body identity, buffering, and the cache-plus-sink restriction;
...
- admission policies, stores, waiting, 429 cooldowns, and multi-policy reservation behavior. Include a `RateLimiter::resolveKeyScopeUsing()` example restricted to limiter names beginning with `saloon:`, a tenant-plan policy resolved from the current PendingRequest, and a service-wide `globally()` policy consumed by the same operation. Show when `resolveRateLimitCooldownKey()` should add provider account or credential identity and when `resolveRateLimitCooldown()` should enforce a provider-defined maximum;
...
- organizing reusable SDK methods with ordinary resource classes typed to their concrete connector; explain that the upstream `BaseResource` is omitted because it weakens connector and DTO typing;
...
2. **HTTP owner fixes.** Reject unsafe connection-cap options, isolate registered asynchronous handlers while preserving synchronous reuse, correct safe named-connection handler options, restore redirect option re-enablement, make URI-template expansion opt-in, correct post-mutation body preparation, nullable raw-body content types, scalar object decoding, the response-exception factory, tests, and HTTP docs first so the Saloon sender can rely on the final transport contract.
...
9. **Cooldown and rate limits.** Add and verify Cooldown across every framework store, then implement operation-aware Saloon policies, stable named limiter scopes, 429 handling, and queue release middleware on it.
10. **Console and package wiring.** Port commands/stubs, finish the minimal README and both package metadata locations, then verify generated output.
11. **Completeness and code gates.** Compare every upstream source/test symbol against the inventory; verify removed framework-neutral/deprecated classes have no consumers, no references rooted in the upstream `Saloon\` or `Illuminate\` namespaces, and no Laravel adapter, promise, sender factory, plugin driver, OAuth 1, mutable connector request/debug/cache/auth/retry state, mutable OAuth setters, or stale namespace references remain. Complete targeted checks, self-review, and `composer fix` before starting the guide.
12. **Documentation.** After the code and full gate are green, agree the guide structure with the package owner, write `src/docs/saloon.md` in Laravel documentation prose against the final API, and add it to navigation. Verify every example against the implemented signatures.
...
- Worker-reused connectors with fixed configuration, explicitly constructed tenant connectors with isolated readonly configuration, connector injection into a default worker-shared controller, fresh no-argument/default-argument Request resolution through `newInstance()`, fail-fast bare and parameterized container resolution plus direct/Saloon `make()`/explicit-binding construction for required arguments, honored explicit bindings, side-effect-free pending construction, independent cloning of every initialized mutable request repository/pipeline/cookie/retry/cache-control value, and repeated sends that create independent pending state.
- Property merge precedence, authentication, the full shared Hypervel HTTP fluent surface, redirect call-order behavior across boolean and array option forms without deprecations or lost sibling settings, all supported methods including body-bearing `QUERY`, HTTP(S)-only base/endpoint joining, base query preservation alone and before endpoint pairs, repository replacement of base and endpoint query families (duplicates, zero/false/empty-string names and values, null values that remove the matched raw key rather than emitting an empty value, valueless pairs, exact and nested-family overrides, literal bracket and brace values, plus/percent-encoded names, nested/Arrayable/JsonSerializable/Stringable/non-finite values), header normalization, body formats, structured JSON/form/query normalization, string `"0"`, empty/null streams, custom JSON flags, connector and request multipart defaults through the live repository-snapshot path, multipart boundaries/metadata/numeric normalization, one-time prepared-body identity across fake/cache/network paths, stream ownership, repeated cookie groups with explicit domains, accepted transport options, rejected request-shaping/authentication/error options, TLS/certificates, Telescope controls, stable base-URL override resolvers, final PSR middleware replacement, and one HTTP attempt per Saloon attempt.
- Response inheritance without duplicated framework methods, scalar/array/object JSON, invalid JSON, seekable and lazily buffered non-seekable streams, retained framework response metadata, exception-safe path/resource export and ownership, inherited reason/header/status/ArrayAccess helpers, and response-data-dependent custom response selection (including a resolver that buffers a non-seekable body before selecting a subclass). Cover request-first custom failure verdicts, request/connector disagreement, status fallback, `onError()`, `AlwaysThrowOnErrors`, successful-plus-failed responses, literal status helpers versus failure-policy helpers, custom/general/client/server exception selection, truncation, DTO runtime behavior and static type inference, fatal exception chains, cached/mocked false resets, and XML/DOM.
...
- Middleware ordering/names, request mutation and fake short-circuiting without PendingRequest replacement, authoritative response replacement, fixed terminal-phase ordering after ordinary request middleware, plugin boot order/inheritance, cached metadata, global boot middleware, listener guards, retries/backoff, per-attempt rematerialization after seekable prepared/final-hook stream and multipart restoration, non-seekable retry rejection, and `Sleep` fakes. Prove that returning a distinct PendingRequest cannot divert body preparation or a later fake from the original operation, and that class-based request middleware can return a fake.
...
- Exact/class/wildcard/callback matching, sequences, exhaustion, strict unmatched behavior, reversible all/URL-limited partial fakes through `preventStrayRequests()` / `allowStrayRequests()`, normal cache/network continuation for permitted unmatched requests, framework-level `Http::preventStrayRequests()` protection when no Saloon mock client is active, request recording, class- and union-typed assertion callbacks, pooled recording, fake precedence over an existing cache entry, and the distinct Saloon-versus-HTTP fake recorders/flags.
- Nested and query-derived valid fixture names, rejected traversal/absolute/backslash/dot-segment names, atomic writes, missing behavior, valid redaction, fail-closed malformed and execution-failing redaction patterns with no file publication, merge/through hooks, and parallel temp-directory isolation. Saloon fakes and cache hits must not populate or refresh cache entries.

### OAuth 2 and security

- Immutable OAuth config construction/memoization without setters, every grant, refresh, PKCE, normal and exact-bound expiry durations, rejection of an overflowing expiry duration, state generation/validation, authorization-query falsey values/owned-key precedence, absolute endpoint policy, sensitive signatures, and concurrent authorization flows on one connector.
- The returned AuthorizationUrl keeps URL/state paired and no connector state remains.

### Cache

- Request-over-connector configuration, request-only cache controls over a stable caching connector, connector/request key hooks, package/framework default store selection, duration types, GET/HEAD/OPTIONS/QUERY defaults, QUERY prepared-body distinction, supported-method overrides, successful-only writes, hits, false cached reset, invalidation, and expiry.
...
- Queue middleware pass-through and return preservation, release for a denied decision's reported delay without propagation, and unchanged propagation of other exceptions.
- Retry-After seconds, every HTTP-date form including both space-padded and two-digit asctime days, mismatched weekdays, RFC 850 year resolution, leap seconds, invalid calendar/time fields, invalid/past values, tenant-scoped cooldowns under the same named limiter, explicit provider account/credential cooldown keys, provider-specific cooldown detection and maximums, shared 429 cooldown within one resolved scope, cooldown-before-admission ordering, no cooldown-recursive resend, explicit retries passing through the recorded cooldown, and next-operation wait/fail behavior.
...
- Every generator/list command, prompt, inherited `--target-path` and `--target-namespace` override, independent custom integration path and namespace, stub publication, and generated syntax.
