...
- A policy scope that adds a join changes the outer query used by `whereCan`. Under Eloquent's normal `select *` behavior, joined columns are hydrated onto the model and duplicate names can replace model attributes, including its primary key. Gate must not force a model-only selection because that would overwrite a caller's deliberate `select()`. Applications hydrating a joining `whereCan` query should select the model's own columns, such as `select('posts.*')`. Scope-derived `withCan` selections are unaffected because their joins remain inside the correlated `EXISTS` query.
...
The documented `*Scope` contract is set membership: add constraints that define which model keys are authorized. Predicates and joins are the normal shape; grouping and `having` are valid only when the resulting query remains a valid correlated `EXISTS` query. A join applied through `whereCan()` is part of the outer query, so callers hydrating models should select the model's own columns to avoid Eloquent hydrating joined columns or replacing attributes with duplicate names. A join used to derive `withCan()` remains inside the correlated subquery and does not affect the outer selection. Do not promise correct annotation derivation for replacement builders, unions, limit/offset windows, outer queries whose `from` clause aliases the model table, or authorization that depends on result ordering. Eloquent's qualified key comes from the model table rather than parsing an aliased `from` clause, so an outer table alias cannot be correlated safely without brittle SQL introspection.
...
- Cover the end-to-end boundaries that unit SQL-shape tests cannot prove:
  ...
  - an exact 63-byte explicit result alias hydrates under its complete name and receives its boolean cast;
  - one joining policy scope filters through `select('auth_query_posts.*')->whereCan()` without leaking joined columns or replacing the model key, and derives a `withCan()` result with its join and join-clause binding intact;
...
- Explain that a joining policy scope changes the outer `whereCan()` query, so model hydration should select the model's own columns to avoid joined attributes and duplicate-name replacement. State that `withCan()` is unaffected because the join stays inside its authorization subquery.
...
- **Forcing a model-only selection from `Gate::scope()`:** a joining policy scope has standard Eloquent join hydration behavior, but Gate cannot replace a caller's deliberate column selection. Applications that hydrate a joining `whereCan()` query select the model's own columns explicitly; `withCan()` needs no such selection because the join stays inside its correlated subquery.
