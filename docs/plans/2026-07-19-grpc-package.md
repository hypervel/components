...
The receive coroutine's ID is also a native-socket ownership token. Swoole forbids closing a coroutine socket from one coroutine while another remains blocked in its read wait. `Coroutine::create()` can run the receiver to completion before returning, so store the returned ID only when `Coroutine::exists()` confirms that receiver survived synchronous startup. This check is safe only in the no-yield window immediately after `create()` returns, where the ID cannot have been recycled. Clear the stored ID on every receiver exit before Swoole can reuse that integer for an unrelated coroutine. Later termination trusts that maintained null-iff-alive invariant and must not call `exists()` after arbitrary intervening yields, where it could match a recycled ID. When termination runs outside the receiver, publish terminal connection/call state first, synchronously cancel the owned receiver without injecting `CanceledException`, then close the engine client after the receiver has unwound from `recv()`. When termination already runs inside the receiver—receive failure or idle retirement—skip self-cancellation and close directly because that coroutine is already the sole socket owner. Do not wait for the one-second poll or add a shutdown channel: explicit `close()` remains synchronous, and the receiver ID is the direct minimal handoff.
...
| `ConnectionTest` | lazy factory settings bounded by the current deadline; fresh request finalization after connect; whole-send/write serialization under concurrent coroutines; mixed-deadline send-semaphore contention and early-wake-safe deadline acquisition; per-operation native timeout publication for deadline/no-deadline sends and writes; deadline-vs-connection failure classification; healthy connection reuse when expiry occurs before native send; receiver-wins/sender-wins pending-state registration races and mismatched/unsolicited IDs; no orphan-event buffer; incremental routing by stream ID; one-second idle polling; receiver-side expiry/retirement of an unobserved deadline; a queued final event can complete the receiver during synchronous startup without retaining its ID; swallowed RST detected through `isStreamOpen()` and isolated to one call; no implicit keepalive pings; non-timeout connection failure fan-out; retiring connection preserves healthy calls, accepts no new calls, closes when only tombstones remain, and releases abandoned buffers; external close cancels a live receiver before native socket close without leaking its cancellation/engine error; receiver-owned retirement skips self-cancellation; coroutine ID ownership is cleared before reuse; slot replacement; receiver stops/restarts with active streams; deterministic owned-channel close; no implicit replay |
