...
The receive coroutine must never block on one slow call and prevent every other multiplexed stream from progressing. State mutation and waiter registration/removal do not wait for channel capacity. A consumer checks its predicate, registers a fresh capacity-one channel, rechecks to close the registration race, then pops only for the remaining deadline. No yield occurs between registration and that pop, so every waiter visible to the receiver is already suspended on an empty channel. Each state change can therefore signal every current waiter without waiting for capacity. Swoole synchronously resumes a waiting coroutine before `push()` returns, so every signal is a re-entrancy boundary: publish terminal/shared state before signaling and recheck connection state after each signal site. Every consumer removes/closes its own channel in `finally`. This supports concurrent metadata/status observers without a missed edge or a permanently retained waiter.
...
`register()` registers the install command when running in console mode, and `boot()` registers publish groups. Route loading belongs to the dedicated listener bootstrap rather than provider boot: after application bootstrap and middleware synchronization, `Server::bootstrapForServer()` directly requires the configured gRPC route file and then validates and compiles the isolated router. The direct load remains independent of global HTTP route caching, while unrelated console commands, queue workers, and schedulers no longer instantiate an isolated route collection they cannot serve.
...
Do not expose `app.debug` exception messages over gRPC. The configured exception reporter retains diagnostics; if it fails, `ExceptionMapper` best-effort writes the original failure to PHP's error log without replacing the RPC status or escaping the transport boundary.
...
Finally, remove `Foundation\Bootstrap\HandleExceptions::renderHttpResponse()`. It is Laravel-SAPI machinery that cannot work in Hypervel: the global handler has no native Swoole response, and `Hypervel\Http\Response::send()` intentionally throws. `handleException()` continues to report first and retains console rendering/exit behavior. For a non-console worker it returns after reporting, allowing the failed coroutine/callback to terminate without a second exception. Catch any `Throwable` from resolving or invoking the reporter rather than only `Exception`, because this is the process's last exception backstop and an `Error` from the reporter must not recursively fault; best-effort write the original throwable to PHP's error log when that reporting step fails.
...
| `ExceptionMapperTest` | expected RPC not reported; rich details; custom error trailers; retry-after rounding/zero and no-retry pushback; oversized error trailers compact to ResourceExhausted; generic → reported Unknown; a failing configured reporter falls back to PHP's error log without replacing the mapped status; protocol mappings; wrong return → reported Internal; no debug leakage; unknown method Unimplemented |
...
| `GrpcServiceProviderTest` | defaults/client-only no port; enabled port shape; non-empty name; default/replaceable health-provider binding; unknown TLS keys and provider-owned raw protocol/TLS settings rejected; valid raw settings preserved; TLS; config/routes publishing; router/context bindings; provider boot does not load routes, while listener bootstrap loads them independently of the global HTTP route cache; the appended listener participates in final `ServerConfig` uniqueness validation |
...
- escaped ordinary response-callback failures are reported and best-effort complete a writable native response; if the configured reporter fails, the original callback failure is written to PHP's error log and the reporter failure still does not escape;
...
- a `Throwable` raised by the reporter does not create a second uncaught exception at the global backstop, and the original failure is written to PHP's error log.
