...
class StartSsr extends Command
{
    ...
    {
        if (! config()->boolean('inertia.ssr.enabled')) {
        ...
        $runtime = $this->option('runtime') ?? config()->string('inertia.ssr.runtime');

        if (config()->boolean('inertia.ssr.ensure_runtime_exists') && ! (new ExecutableFinder)->find($runtime)) {
