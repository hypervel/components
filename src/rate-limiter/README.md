...
- The `consume`, `consumeMany`, `inspect`, `attempt`, `recordFailure`, and `clear` methods replace Laravel's split primitive counter API. Dedicated Redis, Swoole, database, and worker-array stores perform their state changes atomically.
...
- Weighted denials report the actual unused capacity. The `attempt()` method consumes before invoking its callback and retains the charge if the callback throws.
