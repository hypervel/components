...
- [Other Response Types](#other-response-types)
    - [No Content Responses](#no-content-responses)
...
Typically, you won't just be returning simple strings or arrays from your route actions. Instead, you will be returning full `Hypervel\Http\Response` instances or [views](/docs/{{version}}/views).
...
You may also return [Eloquent ORM](/docs/{{version}}/eloquent) models and collections directly from your routes and controllers. When you do, Hypervel will automatically convert the models and collections to JSON responses while respecting the model's [hidden attributes](/docs/{{version}}/eloquent-serialization#hiding-attributes-from-json):
...
By default, thanks to the `Hypervel\Cookie\Middleware\EncryptCookies` middleware, all cookies generated by Hypervel are encrypted and signed so that they can't be modified or read by the client. If you would like to disable encryption for a subset of cookies generated by your application, you may use the `encryptCookies` method in your application's `bootstrap/app.php` file:
...
Redirect responses are instances of the `Hypervel\Http\RedirectResponse` class, and contain the proper headers needed to redirect the user to another URL. There are several ways to generate a `RedirectResponse` instance. The simplest method is to use the global `redirect` helper:
...
When you call the `redirect` helper with no parameters, an instance of `Hypervel\Routing\Redirector` is returned, allowing you to call any method on the `Redirector` instance. For example, to generate a `RedirectResponse` to a named route, you may use the `route` method:
...
If you are redirecting to a URL that may be influenced by user input, you may use the `enforceSameOrigin` method to ensure the final redirect target has the same origin as the current request. If the target does not match, the response will redirect to the fallback URL instead:

```php
return redirect()->away($url)->enforceSameOrigin('/dashboard');
```

By default, `enforceSameOrigin` checks the host, scheme, and port. You may disable the scheme or port checks individually:

```php
return redirect()->away($url)->enforceSameOrigin(
    '/dashboard',
...
The `response` helper may be used to generate other types of response instances. When the `response` helper is called without arguments, an implementation of the `Hypervel\Contracts\Routing\ResponseFactory` [contract](/docs/{{version}}/contracts) is returned. This contract provides several helpful methods for generating responses.

<a name="no-content-responses"></a>
### No Content Responses

You may use the `noContent` method to generate a response with no body and a `204` status code:
...
You may also create a JSONP response directly using the `jsonp` method:

```php
$callback = (string) $request->input('callback');
...
For convenience, if the closure you provide to the `stream` method returns a [Generator](https://www.php.net/manual/en/language.generators.overview.php), Hypervel will automatically flush the output buffer between strings returned by the generator, as well as disable Nginx output buffering:
...
Streamed responses may be consumed using the `@laravel/stream-*` npm packages, which provide a convenient API for interacting with Hypervel response and event streams. To get started, install the `@laravel/stream-react`, `@laravel/stream-vue`, or `@laravel/stream-svelte` package:
...
Then, `useStream` may be used to consume the event stream. After providing your stream URL, the hook will automatically update the `data` with the concatenated response as content is returned from your Hypervel application:
...
Event streams may be consumed using the `@laravel/stream-*` npm packages, which provide a convenient API for interacting with Hypervel event streams. To get started, install the `@laravel/stream-react`, `@laravel/stream-vue`, or `@laravel/stream-svelte` package:
...
Then, `useEventStream` may be used to consume the event stream. After providing your stream URL, the hook will automatically update the `message` with the concatenated response as messages are returned from your Hypervel application:
...
return response()->streamDownload(function () {
    echo GitHub::api('repo')
        ->contents()
        ->readme('hypervel', 'hypervel')['contents'];
}, 'hypervel-readme.md');
```
