...
class Task extends Prompt
{
        ...
        try {
            ...
            if ($this->pid === 0) {
                ...
                if ($this->socket !== null) {
                    ...
                    fwrite($this->socket, $this->identifier . '_reset:' . ($originalAsync ? 1 : 0) . PHP_EOL);
     ...
     *
     * Uses shared memory instead of socket IPC — safe because Swoole
     * coroutines are cooperatively scheduled within the same thread.
     *
     * @template TReturn of mixed
     *
     * @param Closure(Logger): TReturn $callback
