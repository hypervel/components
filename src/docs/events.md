...
If your queue connection's `after_commit` configuration option is set to `false`, you may still indicate that a particular queued listener should be dispatched after the open parent database transactions have committed by implementing the `ShouldQueueAfterCommit` interface on the listener class:
...
> [!WARNING]
> Debouncing is a best-effort way to coalesce work, not a guarantee that only one matching listener will run. Concurrent dispatch and cleanup across multiple servers may allow more than one matching listener to run, and dispatch during middleware may extend an active `maxWait` window. Debounced listeners should be idempotent.
