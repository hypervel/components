...
A non-zero `read_timeout` is applied both when the Redis socket is opened and as the PhpRedis `Redis::OPT_READ_TIMEOUT` option. A value of `0.0` leaves PHP's `default_socket_timeout` in effect. A non-zero `tcp_keepalive` enables TCP keepalive on the Redis socket. PhpRedis does not apply TCP keepalive to Redis Cluster node connections, so Cluster connections reject a non-zero `tcp_keepalive` value and any `tcp_keepalive` entry in their options. The optional `name` value sets the client name on standalone Redis connections.
