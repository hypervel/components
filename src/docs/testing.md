...
This base runs each test method inside a coroutine, provides environment-variable helpers, and handles exception-handler state and the Mockery lifecycle without booting an application. Package tests that need the container, configuration, service providers, database, routes, or an application filesystem should instead extend [`Hypervel\Testbench\TestCase`](/docs/{{version}}/testbench#getting-started).
