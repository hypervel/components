...
Terminal queue correlation normally uses the exact final encoded payload without decoding it. If another finalizer rewrites that payload afterward, the package performs one terminal decode to recover Hypervel's job UUID. A later finalizer that removes or corrupts the UUID, or throws after earlier messages in the same batch were finalized, has broken the framework payload lifecycle. Coroutine or process exit releases those unmatched states; a long-running non-coroutine custom process that catches and continues after such failures may retain them until it exits.
