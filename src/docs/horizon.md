...
If the `defaults` option is omitted, Horizon applies no shared supervisor options.
...
If either silencing option is omitted, Horizon treats it as an empty list.
...
If either retention value is omitted, Horizon keeps 24 snapshots for that metric type. The optional `metrics.snapshot_lock` value prevents overlapping snapshot runs and defaults to 300 seconds.
