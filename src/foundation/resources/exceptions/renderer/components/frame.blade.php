...
>
    ...
    >
        ...
        <div class="flex flex-1 items-center justify-between gap-6 min-w-0">
            <x-hypervel-exceptions-renderer::formatted-source :$frame />
            <x-hypervel-exceptions-renderer::file-with-line :$frame direction="rtl" />
        </div>
    ...
    @if($snippet = $frame->snippet())
        <x-hypervel-exceptions-renderer::frame-code :code="$snippet" :highlightedLine="$frame->line()" x-show="expanded" :x-cloak="!$frame->isMain()" />
    @endif
</div>
