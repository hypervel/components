...
<div id="previous-exceptions" class="flex flex-col gap-2.5 bg-neutral-50 dark:bg-white/1 border border-neutral-200 dark:border-neutral-800 rounded-xl p-2.5 shadow-xs">
    <div class="flex items-center gap-2.5 p-2">
        <div class="bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-white/5 rounded-md w-6 h-6 flex items-center justify-center p-1">
            <x-hypervel-exceptions-renderer::icons.alert class="w-2.5 h-2.5 text-blue-500 dark:text-emerald-500" />
        </div>
        <h3 class="text-base font-semibold text-neutral-900 dark:text-white">Previous {{ \Hypervel\Support\Str::plural('exception', $exception->previousExceptions()->count()) }}</h3>
    </div>
            ...
            <div class="flex gap-2.5 px-2">
                ...
                >
                    ...
                    >
                        ...
                        >
                            <x-hypervel-exceptions-renderer::icons.chevrons-down-up x-show="expanded" />
                            <x-hypervel-exceptions-renderer::icons.chevrons-up-down x-show="!expanded" x-cloak />
                        </button>
                    </div>
                    ...
                    <div x-show="expanded" x-cloak class="flex flex-col gap-1.5 p-3">
                        @foreach ($previous->frameGroups() as $group)
                            @if ($group['is_vendor'])
                                <x-hypervel-exceptions-renderer::vendor-frames :frames="$group['frames']" />
                            @else
                                @foreach ($group['frames'] as $frame)
                                    <x-hypervel-exceptions-renderer::frame :$frame />
                                @endforeach
                            @endif
                        @endforeach
                    </div>
